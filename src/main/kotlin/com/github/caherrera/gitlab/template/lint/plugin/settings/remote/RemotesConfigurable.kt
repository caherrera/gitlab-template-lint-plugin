...
import com.github.caherrera.gitlab.template.lint.plugin.extensions.*
...
class RemotesConfigurable(project: Project) : BoundConfigurable(message("settings.remotes.group.title")) {
    ...
    private fun createTableModel(): ListTableModel<Remote> = ListTableModel(
        arrayOf(
            ...
            createColumn(message("settings.remote.gitlab-alias")) { remote -> remote.gitlabAlias.orEmpty() },
    ...
    private class RemoteDialog(val gitlabUrls: Set<String>, newRemote: Remote? = null) : DialogWrapper(false) {
        ...
        override fun createCenterPanel() = panel {
            ...
            row(message("settings.remote.gitlab-alias")) {
                comboBox(gitlabUrls)
                    .align(Align.FILL)
                    .bindItem(remote::gitlabAlias.toNullableProperty())
                    .resizableColumn()

                contextHelp(message("settings.remote.gitlab-alias.context-help"))
                    .align(AlignX.RIGHT)
            }
